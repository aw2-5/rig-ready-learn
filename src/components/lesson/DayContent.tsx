    ...
    <div className="space-y-4 animate-fade-in">
      ...
      {practiceQuestion && (
        <Card variant="accent">
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              {practiceQuestion.options.map((option, index) => {
                ...
                return (
                  <Button
                    key={index}
                    variant={showCorrect ? 'default' : showWrong ? 'destructive' : isSelected ? 'accent' : 'secondary'}
                    className={`w-full justify-start text-start h-auto py-3 px-4 ${
                      showCorrect ? 'bg-green-500/20 border-green-500 text-green-500' : ''
                    }`}
                    onClick={() => !showPracticeResult && handlePracticeAnswer(index)}
            ...
            {showPracticeResult && (
              <p className={`mt-3 text-sm font-medium ${isCorrect ? 'text-green-500' : 'text-destructive'}`}>
      ...
      {!isComplete && (
        <Button 
          ...
          className="w-full" 
          onClick={onComplete}
          disabled={practiceQuestion && !showPracticeResult}
        >
